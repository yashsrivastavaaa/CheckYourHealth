...
def lung_cancer(list):
    dataset = pd.read_csv('lungs_cancer.csv') # IMPORTING THE DATASET
    ...
    dataset.groupby('LUNG_CANCER').mean()

    without_class = dataset.drop(columns = 'LUNG_CANCER' , axis = 1)
    only_class = dataset['LUNG_CANCER']
    print(only_class)
    ...
    if(predicted_data[0] == 0):
      print("No Lungs CANCER Issue")
      return 0
    else:
      print("May Have Lungs Cancer Issue")
    return 1